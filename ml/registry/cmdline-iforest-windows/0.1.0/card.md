# cmdline-iforest-windows 0.1.0 — Model Card
...
- **Status**: **reference-only, NOT shippable** — migration artifact from the old
  iteration (old/ml/model_windows.onnx, trained 2026-08 by train_windows.py on a Windows lab
  baseline capture). It predates the registry's evaluation gates.
- **Training data**: Windows lab baseline capture (old iteration; dataset not versioned
...
- **Ship criteria**: a successor trained via synthaea_ml/training/train_windows.py on a
