...
//! - **Inline blocking** (`-EPERM` on a policy verdict). `crates/response` and
//!   `crates/policy`'s verdict model don't exist yet (issues #131, #133) — there is
//!   nothing for a block decision to consult. Wiring blocking ahead of that would
//!   mean inventing the verdict contract here first, unreviewed.
...
//! - **Lab validation.** [`detect_hook_support`] returns `true` on this machine's
//!   kernel (WSL2, `6.6.114-microsoft-standard`) — the BTF type is compiled in — but
//!   `attach_file_open` needs a loaded eBPF object (root) to test for real, and
//!   `/sys/kernel/security/lsm` doesn't exist here to confirm "bpf" is even a
//!   registered LSM. Concretely: BTF presence alone did NOT prove attach works on
//!   this box, which is exactly why [`detect_hook_support`] is documented as
//!   necessary, not sufficient. The three `Done when` items on #91 (`io_uring` write
//!   observed, policy-gated block, capability reporting on a kernel without
//!   `lsm=bpf`) all need the Hyper-V lab kernels either way.
