//! The linux-only implementation: loading, attaching, and reading the `file_open`
//! BPF-LSM hook via `aya`. Everything `aya`-shaped lives here so the crate
//! compiles to an empty stub on other platforms (the workspace-wide sensor rule).
