...
//!   synthaea-session` one-liner kill); every session matching our `wtrace-`
//!   prefix is enumerated and stopped at startup (issue #408 — catches every
//!   orphan left by any number of consecutive unclean shutdowns, not just the
//!   most recent), and a silence watchdog turns a stopped trace into a loud
//!   sensor error instead of quiet blindness.
